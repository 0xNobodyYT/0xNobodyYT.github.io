(() => {
  ...
  function renderGrid() {
    ...
    $('companionGrid').innerHTML = list.length ? list.map(c => `<button class="companion-card ${c.premium?'premium':''} ${c.id===state.selected?'active':''}" data-id="${c.id}">${c.art.icon?`<img src="${esc(c.art.icon)}" alt="${esc(c.name)} portrait">`:`<span class="card-fallback" title="Portrait is not available in the current global client">${esc(initials(c.name))}</span>`}<span><strong>${esc(c.name)}</strong><small>${c.premium?'Premium':'Standard'} · ${esc(c.nativeClass)}</small><small class="mini-stats">${esc(topSpecials(c))}</small>${c.availability!=='Global client'?`<small class="future-note">${esc(c.availability==='TW / future'?'TW / future':'Global unreleased')}</small>`:''}</span></button>`).join('') : '<p class="empty-results">No companions match these filters.</p>';
  ...
  function renderDetail() {
    ...
    $('badges').innerHTML = `<span class="badge ${c.premium?'premium':''}">${c.premium?'Premium':'Standard'}</span><span class="badge">${c.catalog}</span><span class="badge availability">${esc(c.availability||'Global client')}</span>${duplicateNames.has(c.name)?`<span class="badge">Client entry ${c.id}</span>`:''}`;
    ...
    $('recommendationReason').textContent = c.recommendationReason || 'General-purpose bonus profile.';
    ...
    $('figure').src = c.art.figure || c.art.icon || ''; $('figure').alt = c.art.figure || c.art.icon ? `${c.name} artwork` : ''; $('figure').className = c.art.figure ? 'figure-full' : 'figure-icon';
    $('figure').hidden = !(c.art.figure || c.art.icon); $('portraitFallback').hidden = !($('figure').hidden); $('portraitFallback').textContent = initials(c.name); $('portraitFallback').title = 'Portrait is not available in the current global client';
